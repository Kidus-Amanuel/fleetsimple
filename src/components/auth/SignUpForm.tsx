...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.auth.signUp(formData.email, formData.password, fullName);
      setSuccess(true);
      // Supabase typically requires email confirmation depending on your settings
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center flex-1 w-full max-w-md mx-auto text-center">
        <div className="p-8 bg-white rounded-2xl shadow-theme-xl dark:bg-gray-900">
          <div className="flex items-center justify-center w-16 h-16 mx-auto mb-6 bg-green-100 rounded-full dark:bg-green-500/10">
            <svg className="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
            </svg>
          </div>
          <h2 className="mb-2 text-2xl font-bold text-gray-800 dark:text-white/90">Registration Successful!</h2>
      ...
      <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto ">
        <div>
          ...
          <div>
            ...
            {error && (
              <div className="p-3 mb-4 text-sm text-red-500 bg-red-100 border border-red-200 rounded-lg dark:bg-red-500/10 dark:border-red-500/20">
                {error}
              </div>
            )}
